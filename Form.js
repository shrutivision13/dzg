...
import { Autocomplete, Checkbox, Dialog, DialogContent, DialogTitle, FormControl, FormLabel, IconButton, Menu, Paper } from '@mui/material';
function Form({ param }) {
			...
			if (res.success) {
				res.data.map((el) => {
					item.push({
						...
						label: el.customer.customerNumber + ' - ' + el.customer.email,
							...
							<Autocomplete
								...
								onInputChange={(e, newInputValue) => {
									if (e != null) {
										...
										if (newInputValue.length >= 3 && !isDisabled) {
											// setValue('customer.customerNumber', existingOption?.value);
											setValue('customer.email', existingOption?.email);
											setValue('customer.contactPerson', existingOption?.contactPerson);
										}
								...
								onChange={(e, selectedOption) => {
									if (selectedOption && !isDisabled) {
										...
										setValue('customer.contactPerson', selectedOption.contactPerson);
																					...
																					<Controller
																						...
																						render={({ field }) => (
																							<>
																								...
																								{
																									field.value &&
																									<img src={field.value} alt="Preview" className="image_preview" id="preview_image" />
																								}
																							</>
																						)}
																					/>
...

											{
												localStorage.getItem("isWriterView") ==
													...
													<Grid container xs={12} sx={{ p: 1 }} justifyContent="center" item>
														...
														{
															localStorage.getItem("isCopy") && localStorage.getItem("isCopy") === "true" ?
																<></>
																:
																<Button
																	variant="contained"
													...
													<Grid container xs={12} sx={{ p: 1 }} justifyContent="center" alignItems='center' item>
														...
														{
															localStorage.getItem("isWriter") ? <Box className="form_edit">
																<Button
																	variant="contained"
																	onClick={() => setisDisabled(false)}
																>
